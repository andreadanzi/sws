...
function Contacts_sendCustomerPortalLoginDetails($entityData){
    global $log;
    $log->debug("Contacts_sendCustomerPortalLoginDetails started!");
    ...
    $log->debug("Contact id ".$entityId);
    $log->debug("Contact email ".$email);
    $log->debug("Contact entityData portal ".$entityData->get('portal'));
	if ($entityData->get('portal') == 'on' || $entityData->get('portal') == '1') {
		...
		if ($insert == true) {
            $log->debug("Contacts_sendCustomerPortalLoginDetails insert true");
            ...
            $log->debug("Contacts_sendCustomerPortalLoginDetails insert sql ". $sql);
            $log->debug("Contacts_sendCustomerPortalLoginDetails insert params ". $params);
    ...
    $log->debug("Contacts_sendCustomerPortalLoginDetails terminated!");
}
