...
function send_mail_for_password($mailid)
{
	...
	$sql = "SELECT * FROM 
		...
		vtiger_portalinfo
		WHERE 
		vtiger_portalinfo.id = vtiger_contactdetails.contactid
		AND user_name = ? ";
	$res = $adb->pquery($sql, array($mailid));
	$cust_id = $adb->query_result($res,0,'id');
	...
	$resQ = $adb->query_result($res, 0, 'cryptmode');
	// We no longer have the original password!
	if ($resQ && !empty($resQ)) {
		// danzi.tn@20161201 send new password
		...
		$enc_password = Vtiger_Functions::generateEncryptedPassword($password);
                ...
                $params = array($enc_password, 'CRYPT', $cust_id);
	...
	$contents .= "<br><br>Dear ".$firstname." ".$lastname.", you request a new password to access the SWS Customer Portal. Please find below your credentials:<br><br>".getTranslatedString('LBL_USERNAME')." ".$user_name;
	$contents .= "<br>".getTranslatedString('LBL_PASSWORD')." ".$password;

	$mail = new PHPMailer();

	$mail->Subject =  "SWS Engineering: " . getTranslatedString('LBL_SUBJECT_PORTAL_LOGIN_DETAILS');
	$mail->Body    = $contents;
	$mail->IsSMTP();
